...
- Cek apakah ada perubahan terakhir di kode/config fitur ini sebelum bug muncul. Kalau ada backup dari sebelum bug muncul (lihat "Sumber data berupa backup" di `00-router.md`), diff file terkait fitur ini (shortcode/handler/hook) vs backup lama buat langsung nemuin baris yang berubah, daripada nebak dari trace kode doang.
